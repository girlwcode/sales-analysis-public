...
#Index(['S. No.', 'Month', 'Client', 'City', 'State', 'Region', 'Category',
      ...
      #  'Serial No.', 'Invoice Details', 'Actual Value', 'Tax ', 'Total',
      #  'Payment Status (Rs) ', 'Payment Status', 'Installation Status',
      #  'Warraty', 'installation', '6month visit'],
      # dtype='object')
...
# col에 년도 추가
revenue_17['Year'] = 2017
revenue_18['Year'] = 2018
revenue_19['Year'] = 2019
...
plt.title('Whole Revenue of Inbody,India', fontsize=18)
x = ['2017','2018','2019','2020','2021']
y = [round(sum(revenue_17['Net']),2),round(sum(revenue_18['Net']),2),round(sum(revenue_19['Net']),2),round(sum(revenue_20['Net']),2),round(sum(revenue_21['Net']),2)]
plt.plot(x,y)
...
plt.savefig('../../resource/Plot/Whole Revenue of Inbody,India (Yearly).png')
plt.show()
...
# concat the revenues
df = pd.concat([revenue_17,revenue_18,revenue_19,revenue_20,revenue_21],axis=0,  ignore_index=True)
df.to_csv('../../resource/SalesData/Whole Revenue.csv', index=False)